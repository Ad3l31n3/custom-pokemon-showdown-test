  ...
  },
  {
    ...
    onModifySpecies(species, target, source) {
      ...
      const god = target.side.team.find((set) => {
        ...
        if (typeof setSpecies.battleOnly === "string")
          setSpecies = this.dex.species.get(setSpecies.battleOnly);
        if (set.item && this.dex.items.get(set.item).megaStone) {
          const item = this.dex.items.get(set.item);
        ...
        if (this.ruleTable.has("standardnatdex")) {
          const format = this.dex.formats.getRuleTable(this.dex.formats.get("gen8nationaldex"));
          if (format.isBannedSpecies(setSpecies))
            return true;
        } else {
  ...
  },
  {
    ...
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        ...
        pokemon.m.innates = Object.keys(pokemon.species.abilities).map((key) => this.toID(pokemon.species.abilities[key])).filter((ability) => !ruleTable.isBanned(`ability:${ability}`)).filter((ability) => ability !== pokemon.ability);
      }
    },
