...
const Conditions = {
  ...
  radish: {
    // exclusively caused by Radish Punch, Radish Body, and Radish Orb.
    name: "radish",
    effectType: "Status",
    onStart(target, source, sourceEffect) {
      if (sourceEffect && sourceEffect.effectType === "Ability") {
  ...
  acidicrainfall: {
    ...
    onFieldResidual() {
      ...
      if (this.field.isWeather("acidicrainfall"))
        this.eachEvent("Weather");
    },
    onWeather(target) {
      this.damage(target.baseMaxhp / 16);
  ...
  bladerain: {
    ...
    onFieldResidual() {
      ...
      if (this.field.isWeather("bladerain"))
        this.eachEvent("Weather");
    },
    onWeather(target) {
      1;
    ...
    onModifyMove(move, pokemon) {
      ...
      if (move.secondaries) {
        for (const secondary of move.secondaries) {
          if (secondary.chance)
            secondary.chance *= chanceMod;
        }
      }
      if (move.self?.chance)
        move.self.chance *= chanceMod;
    },
