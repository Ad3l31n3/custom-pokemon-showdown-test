...
const Abilities = {
  ...
  colorchange: {
    onAfterMoveSecondary(target, source, move) {
      ...
      if (target.isActive && move.effectType === "Move" && move.category !== "Status" && type !== "???" && !target.hasType(type)) {
        if (!target.setType(type, false, target, this.effect))
          return false;
  ...
  mimicry: {
    ...
    onTerrainChange(pokemon) {
      ...
      if (oldTypes.join() === types.join() || !pokemon.setType(types, false, pokemon, this.effect))
        return;
  ...
  overcoat: {
    onImmunity(type, pokemon) {
      if (type === "sandstorm" || type === "hail" || type === "hyperboreanarctic" || type === "powder")
        return false;
    },
    onTryHitPriority: 1,
    onTryHit(target, source, move) {
  ...
  protean: {
    onPrepareHit(source, target, move) {
      ...
      const type = move.type;
      if (type && type !== "???" && source.getTypes().join() !== type) {
        if (!source.setType(type, false, source, this.effect))
          return;
  ...
  anyability: {
    ...
    onStart(pokemon) {
      ...
      const abilityList = Object.values(this.dex.data.Abilities).filter((ability) => !bannedAbilities.includes(this.toID(ability.name))).filter((ability) => !ability.isNonstandard).map((ability) => this.toID(ability.name));
  ...
  concert: {
    ...
    onResidual(pokemon) {
      ...
      for (const target of targets) {
        if (!target || !target.hp || pokemon === target)
          continue;
  ...
  degradation: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (degradationUser !== this.activePokemon)
        return;
  ...
  pollution: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (pollutionUser !== this.activePokemon)
        return;
  ...
  boardpowera: {
    name: "Board Power (/a/)",
    onTryHit(pokemon, target, move) {
      if (pokemon.baseSpecies.baseSpecies !== "Fontaba")
        return;
      if (move.ohko) {
  ...
  boardpowerco: {
    ...
    onEffectiveness(typeMod, target, type, move) {
      if (!target || target.baseSpecies.baseSpecies !== "Fontaba")
        return;
      if (move.type === "Dark") {
        return -1;
      }
  ...
  boardpowerd: {
    ...
    onModifyMove(move, source) {
      if (source.baseSpecies.baseSpecies !== "Fontaba")
        return;
      const ignoredMoves = ["judgment", "multiattack", "naturalgift", "revelationdance", "technoblast", "weatherball"];
      if (move.type === "Normal" && !ignoredMoves.includes(move.id) && !(move.isZ && move.category !== "Status")) {
        move.type = "Dark";
  ...
  boardpowerpol: {
    ...
    onModifyAtkPriority: 5,
    onModifyAtk(atk, attacker, defender) {
      if (attacker.baseSpecies.baseSpecies !== "Fontaba")
        return;
      if (!defender.activeTurns) {
  ...
  boardpowersoc: {
    ...
    onAnyModifyDamage(damage, source, target, move) {
      if (target.baseSpecies.baseSpecies !== "Fontaba" && this.effectState.target.baseSpecies.baseSpecies !== "Fontaba")
        return;
      if (target !== this.effectState.target && target.isAlly(this.effectState.target)) {
    ...
    onStart(pokemon) {
      ...
      for (const activePokemon of this.getAllActive()) {
        if (activePokemon === pokemon)
          continue;
        if (activePokemon.isSemiInvulnerable())
          continue;
        if (activePokemon.volatiles["substitute"])
          continue;
  ...
  artificial: {
    ...
    onTryAddVolatile(status, pokemon) {
      ...
      if (immuneStatuses.includes(status.id)) {
  ...
  spookyaura: {
    ...
    onAnyBasePowerPriority: 20,
    onAnyBasePower(basePower, source, target, move) {
      if (target === source || move.category === "Status" || move.type !== "Ghost")
        return;
      if (!move.auraBooster)
        move.auraBooster = this.effectState.target;
      if (move.auraBooster !== this.effectState.target)
  ...
  temperamental: {
    onResidual(pokemon) {
      if (pokemon.species.baseSpecies !== "Disbeary" || pokemon.transformed)
        return;
  ...
  horror: {
    ...
    onStart(pokemon) {
      for (const foe of pokemon.foes()) {
        if (foe.hasType("Grass"))
          return false;
        if (!foe.addType("Grass"))
          return false;
  ...
  overeager: {
    ...
    onBasePowerPriority: 7,
    onBasePower(basePower, pokemon, target, move) {
      if (move.multihitType === "overeager" && move.hit > 1)
        return this.chainModify(0.25);
    },
    onSourceModifySecondaries(secondaries, target, source, move) {
  ...
  overeagerest: {
    ...
    onBasePowerPriority: 7,
    onBasePower(basePower, pokemon, target, move) {
      if (move.multihitType === "overeagerest" && move.hit > 1)
        return this.chainModify(Math.pow(move.hit, 2));
    },
    onSourceModifySecondaries(secondaries, target, source, move) {
  ...
  swarming: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Jermin" || pokemon.level < 20 || pokemon.transformed)
        return;
      if (pokemon.hp > pokemon.maxhp / 4) {
    ...
    onResidualOrder: 27,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Jermin" || pokemon.level < 20 || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp > pokemon.maxhp / 4) {
        if (pokemon.species.id === "jermin") {
          pokemon.formeChange("Jermin-Swarm");
  ...
  stoneflesh: {
    onModifyMovePriority: 1,
    onModifyMove(move, attacker, defender) {
      if (attacker.species.baseSpecies !== "Gargarramer" || attacker.transformed)
        return;
      if (move.category === "Status" && move.id !== "petrify")
        return;
  ...
  hydrothermal: {
    name: "Hydrothermal",
    onModifyMove(move) {
      if (!["Fire", "Water"].includes(move.type))
        return;
      if (move.category === "Status")
        return;
      if (!move.secondaries) {
  ...
  frozenbunker: {
    onDamagingHit(damage, target, source, move) {
      if (!move.damage && !move.damageCallback && target.getMoveHitData(move).typeMod > 0) {
        if (target.baseSpecies.baseSpecies !== "Arctiglobe" || target.transformed)
          return;
  ...
  niceface: {
    ...
    onDamagePriority: 1,
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && effect.category === "Physical" && target.species.id === "blobbosnice" && !target.transformed) {
    ...
    onCriticalHit(target, type, move) {
      if (!target)
        return;
      if (move.category !== "Physical" || target.species.id !== "blobbosnice" || target.transformed)
        return;
      if (target.volatiles["substitute"] && !(move.flags["bypasssub"] || move.infiltrates))
        return;
      if (!target.runImmunity(move.type))
  ...
  zenmonke: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Simionach" || pokemon.transformed)
        return;
    ...
    onResidualOrder: 29,
    onResidual(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Simionach" || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp < pokemon.maxhp) {
        if (pokemon.species.id === "simionach") {
          pokemon.formeChange("Simionach-Zen");
  ...
  memepower: {
    onAfterMove(source, target, move) {
      ...
      if (!move || !target)
        return;
      if (source.ability !== "memepower")
        return;
      if (move.category === "Status")
        return;
      if (source.abilityState.hasMemed?.[targetSlot])
        return;
      if (!source.abilityState?.hasMemed)
        source.abilityState.hasMemed = {};
      source.abilityState.hasMemed[targetSlot] = true;
      this.actions.useMove("meme", source, target);
    },
    onResidual(pokemon) {
      pokemon.abilityState.hasMemed = void 0;
    },
    name: "Meme Power",
    rating: 4.5,
    isNonstandard: "Future"
  ...
  godrejection: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (godrejectionUser !== this.activePokemon)
        return;
  ...
  hyperboreanarctic: {
    ...
    onAnySetWeather(target, source, weather) {
      ...
      if (this.field.getWeather().id === "hyperboreanarctic" && !strongWeathers.includes(weather.id))
        return false;
    },
    onEnd(pokemon) {
      if (this.field.weatherState.source !== pokemon)
  ...
  infection: {
    onDamagePriority: -30,
    onDamage(damage, target, source, effect) {
      if (!["blobbosinfected", "contamination"].includes(target.species.id))
        return;
      if (damage >= target.hp) {
  ...
  costume: {
    onDamagePriority: 1,
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType === "Move" && ["blobbosmimikyu"].includes(target.species.id) && !target.transformed) {
  ...
  turbine: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Flying" || ["firepledge", "grasspledge", "waterpledge"].includes(move.id))
        return;
      const redirectTarget = ["randomNormal", "adjacentFoe"].includes(move.target) ? "normal" : move.target;
      if (this.validTarget(this.effectState.target, source, redirectTarget)) {
        if (move.smartTarget)
  ...
  presentpower: {
    onAfterMove(source, target, move) {
      ...
      if (!move || !target)
        return;
      if (source.ability !== "presentpower")
        return;
      if (move.category === "Status")
        return;
      if (source.abilityState.hasMemed?.[targetSlot])
  ...
  kattapillarssecretpower: {
    ...
    rating: 0,
    // LMAO YOU ACTUALLY THOUGHT THIS SHIT HAD REAL EFFECTS LMAOOOOOOOO
    isNonstandard: "Future"
    // YOU EVEN CHECKED THE CODE HAHAHAHAHAHAHA
  },
  fake: {
    onSwitchIn(pokemon) {
  ...
  metronomepower: {
    onAfterMove(source, target, move) {
      ...
      if (!move || !target)
        return;
      if (source.ability !== "metronomepower")
        return;
      if (move.category === "Status" && (move.id === "metronome" || move.id === "metronomeifitwasfunny"))
        return;
      if (source.abilityState.hasMemed?.[targetSlot])
        return;
      if (!source.abilityState?.hasMemed)
  ...
  copypower: {
    ...
    onStart(pokemon) {
      ...
      if (faintedPokemon) {
        ...
        if (ability.isPermanent || additionalBannedAbilities.includes(ability.id))
          return;
  ...
  onaquest: {
    name: "On A Quest",
    onStart(pokemon) {
      if (pokemon.species.id !== "blobbosadventurer")
        return;
      const quests = [
        { id: "ko", name: "Righteous Purge", requirement: 1, text: "KO 1 Pok\xE9mon", progressText: "Pok\xE9mon KO'd" },
        {
          id: "repeat",
  ...
  taskoriented: {
    ...
    onStart(pokemon) {
      ...
      if (!this.effectState.tasks) {
        const newTasks = [];
        newTasks.push(sampleNoReplace(tasks));
        newTasks.push(sampleNoReplace(tasks));
    ...
    onAfterMove(source, target, move) {
      const repeatTask = this.effectState.tasks.find((task) => task.id === "repeat");
      if (repeatTask) {
        if (!this.effectState.repetition) {
          this.effectState.repetition = { moveId: move.id, times: 1 };
        } else {
      ...
      const moveTask = this.effectState.tasks.find((task) => task.id === "repeat");
      if (moveTask) {
    ...
    onSwitchOut(pokemon) {
      const switchTask = this.effectState.tasks.find((task) => task.id === "switch");
      if (switchTask) {
  ...
  neurotoxin: {
    ...
    onAnyEffectiveness(typemod, target, type, move) {
      ...
      if (neurotoxinUser !== this.activePokemon)
        return;
  ...
  metamorphosis: {
    onSwitchOut(pokemon) {
      if (pokemon.species.id !== "blobboseedle" || pokemon.transformed)
        return;
  ...
  anythingyoucando: {
    ...
    onResidual(pokemon) {
      if (!pokemon.moveThisTurn)
        return;
      for (const target of pokemon.foes()) {
        if (pokemon.moveThisTurn !== target.moveThisTurn)
          return;
        this.damage(target.baseMaxhp / 8, target, pokemon);
      }
    },
  ...
  allaccordingtokeikakuplan: {
    ...
    onFoeSwitchOut(pokemon) {
      if (!this.effectState.switchedPokemon)
    ...
    onBasePower(basePower, pokemon, target, move) {
      ...
      if (!this.effectState.switchedPokemon)
        this.effectState.switchedPokemon = {};
      if (!this.effectState.switchedPokemon[target.position])
        return;
      const previousPokemon = this.effectState.switchedPokemon[target.position];
  ...
  mortal: {
    onUpdate(pokemon) {
      if (pokemon.species.id !== "blobboslichmortal")
        return;
  ...
  immortality: {
    onUpdate(pokemon) {
      if (pokemon.species.id !== "blobboslich")
        return;
      if (!pokemon.side.pokemon.some((ally) => ally !== pokemon && !ally.fainted && !["blobboslich", "blobboslichmortal"].includes(ally.species.id) && ally.item === "phylactery")) {
    ...
    onDamage(damage, target) {
      if (target.species.id !== "blobboslich")
        return;
      if (damage >= target.hp) {
  ...
  joycon: {
    ...
    onModifyMove(move, attacker, defender) {
      if (attacker.species.baseSpecies !== "Blobbos-Switch" && attacker.species.name !== "Blobbos-Switch" || attacker.transformed)
        return;
      if (move.category === "Status") {
  ...
  possessed: {
    onModifyMovePriority: 1,
    onModifyMove(move, attacker, defender) {
      if (attacker.species.name !== "Blobbos-Doll" || attacker.transformed)
        return;
      if (move.id !== "plushrush")
        return;
  ...
  colorboost: {
    ...
    onAfterTypeChange(typeChange, pokemon) {
      if (this.effectState.colorBoost)
        return;
  ...
  ascension: {
    ...
    onDamagePriority: -40,
    onDamage(damage, target, source, effect) {
      if (target.species.id !== "blobboshomestuck")
        return;
      if (source !== target && effect.id !== "recoil")
        return;
      if (damage >= target.hp) {
  ...
  multiversal: {
    ...
    onUpdate(pokemon) {
      ...
      const rawData = (0, import_sim2.FS)("config/multiversal.json").readIfExistsSync() || '{ "step": 0 }';
      ...
      if (this.effectState.step >= data.step) {
        ...
        (0, import_sim2.FS)("config/multiversal.json").writeSync(JSON.stringify(newData));
      } else {
  ...
  acabability: {
    ...
    onStart(pokemon) {
      ...
      if (dayOfWeek === 0) {
        ...
        if (!pokemon.speciesState["parent"]) {
          ...
          const baby = new import_sim.Pokemon({
            ...targetSet,
            name: childName,
    ...
    onAnyTryMove(source) {
      const active = this.getAllActive().some((pokemon) => pokemon.hasAbility("acabability") && pokemon.abilityState && pokemon.abilityState.type === "tripping");
  ...
  aintnothingonnabreakmystride: {
    ...
    isBreakable: false,
    // YOU EXPLICITLY CANNOT BREAK THEIR STRIDE
  ...
  cellconstruct: {
    onResidualOrder: 29,
    onResidual(pokemon) {
      if (pokemon.species.name !== "Blobbos-Mitosis" || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.species.id === "blobbosmitosiscomplete" || pokemon.hp > pokemon.maxhp / 2)
        return;
  ...
  colonization: {
    onStart(pokemon) {
      if (pokemon.level < 20 || pokemon.transformed)
        return;
      if (pokemon.hp > pokemon.maxhp / 4) {
    ...
    onResidualOrder: 27,
    onResidual(pokemon) {
      if (pokemon.level < 20 || pokemon.transformed || !pokemon.hp)
        return;
      if (pokemon.hp > pokemon.maxhp / 4) {
        if (pokemon.species.id === "blobboscolony") {
          pokemon.formeChange("Blobbos-Colony-Colonized");
  ...
  vacuum: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Wind" && move.type !== "Flying" || ["firepledge", "grasspledge", "waterpledge"].includes(move.id))
        return;
      const redirectTarget = ["randomNormal", "adjacentFoe"].includes(move.target) ? "normal" : move.target;
      if (this.validTarget(this.effectState.target, source, redirectTarget)) {
        if (move.smartTarget)
  ...
  dreamcatcher: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Psychic" || ["firepledge", "grasspledge", "waterpledge"].includes(move.id))
        return;
      const redirectTarget = ["randomNormal", "adjacentFoe"].includes(move.target) ? "normal" : move.target;
      if (this.validTarget(this.effectState.target, source, redirectTarget)) {
        if (move.smartTarget)
