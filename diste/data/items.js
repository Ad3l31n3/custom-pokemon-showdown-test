...
const Items = {
  ...
  custapberry: {
    ...
    onFractionalPriority(priority, pokemon) {
      if (priority <= 0 && (pokemon.hp <= pokemon.maxhp / 4 || pokemon.hp <= pokemon.maxhp / 2 && pokemon.hasAbility(["gluttony", "bountifulharvest"]) && pokemon.abilityState.gluttony)) {
        if (pokemon.eatItem()) {
  ...
  eyedropper: {
    name: "Eye Dropper",
    onStart(pokemon) {
      if (pokemon.species.name !== "Blobbos-Chroma")
        return;
      const typeTotals = {};
