  ...
  teampreview: {
    ...
    onBegin() {
      ...
      for (const pokemon of this.getAllPokemon()) {
        const details = pokemon.details.replace(", shiny", "").replace(/(Pumpkaboo)(-[a-zA-Z?-]+)?/g, "$1-*");
  ...
  datamod: {
    ...
    onSwitchIn(pokemon) {
      ...
      if (pokemon.illusion) {
        ...
        if (!pokemon.illusion.m.isModded)
          return;
        this.add("-start", pokemon, "typechange", pokemon.illusion.getTypes(true).join("/"), "[silent]");
        if (pokemon.illusion.m.switchedIn)
          return;
        pokemon.illusion.m.switchedIn = true;
      } else {
        if (!pokemon.m.isModded)
          return;
        this.add("-start", pokemon, "typechange", pokemon.getTypes(true).join("/"), "[silent]");
        if (pokemon.m.switchedIn)
          return;
      ...
      if (target.hasAbility("illusion")) {
        if (target.m.isModded) {
          ...
          if (!target.m.switchedIn) {
            ...
            const type2 = species.types[1];
            this.add(
              `raw|<ul class="utilichart"><li class="result"><span class="col pokemonnamecol" style="white-space: nowrap">${species.name}</span> <span class="col typecol"><psicon type="${type}" />${type2 ? `<psicon type="${type2}" />` : ""}</span> <span style="float: left ; min-height: 26px"><span class="col abilitycol">${abilities}</span><span class="col abilitycol"></span></span></li><li style="clear: both"></li></ul>`
            );
            this.add(
              `raw|<ul class="utilichart"><li class="result"><span style="float: left ; min-height: 26px"><span class="col statcol"><em>HP</em><br>${baseStats.hp}</span> <span class="col statcol"><em>Atk</em><br>${baseStats.atk}</span> <span class="col statcol"><em>Def</em><br>${baseStats.def}</span> <span class="col statcol"><em>SpA</em><br>${baseStats.spa}</span> <span class="col statcol"><em>SpD</em><br>${baseStats.spd}</span> <span class="col statcol"><em>Spe</em><br>${baseStats.spe}</span> </span></li><li style="clear: both"></li></ul>`
            );
          }
        } else {
