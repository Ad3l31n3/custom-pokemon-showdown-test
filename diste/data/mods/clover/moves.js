...
const Moves = {
  ...
  hex: {
    ...
    basePowerCallback(pokemon, target, move) {
      if (target.status || target.hasAbility("comatose") || target.hasAbility("boardpowerz"))
  ...
  metronome: {
    ...
    onHit(target, source, effect) {
      const moves = this.dex.moves.all().filter((move) => (![2, 4].includes(this.gen) || !source.moves.includes(move.id)) && !move.realMove && !move.isZ && !move.isMax && (!move.isNonstandard || move.isNonstandard === "Unobtainable" || this.format.isNonstandard === "CAP" && move.isNonstandard === "CAP") && !effect.noMetronome.includes(move.name));
      let randomMove = "";
      if (moves.length) {
  ...
  wakeupslap: {
    ...
    isNonstandard: null,
    basePowerCallback(pokemon, target, move) {
      if (target.status === "slp" || target.hasAbility("comatose") || target.hasAbility("boardpowerz"))
        return move.basePower * 2;
      return move.basePower;
    }
  },
  ...
  beatup: {
    ...
    damageCallback(pokemon, target, move) {
      let source = pokemon;
      if (move && move.hit > 1) {
        const allies = pokemon.side.pokemon.filter((ally2) => ally2 !== pokemon && !ally2.fainted);
        const ally = allies[move.hit - 2];
        if (ally) {
          source = new import_sim.Pokemon({
            ...ally.set,
            ability: void 0,
            item: void 0
          }, pokemon.side);
