...
class RandomTeams {
  ...
  randomSet(species, teamDetails = {}, isLead = false, isDoubles = false, isCloveronly = false) {
    ...
    const randomBattleSet = cloverSetData && cloverSetData.sets ? this.sample(cloverSetData.sets) : {};
    ...
    do {
      ...
      for (const [k, moveId] of moves.entries()) {
        ...
        switch (moveid) {
          ...
          case "fly":
          case "storedpower":
            if (!hasType[move.type] && !counter.setupType && !!counter.Status)
              rejected = true;
            break;
          case "fireblast":
          ...
          case "futuresight":
            ...
            break;
          case "hypervoice":
            if (hasType["Electric"] && movePool.includes("thunderbolt"))
              rejected = true;
            break;
          case "payback":
          case "psychocut":
          ...
          case "pollenpuff":
            ...
            break;
          case "defog":
            if (counter.setupType || hasMove["healbell"] || hasMove["toxicspikes"] || teamDetails.defog)
              rejected = true;
            break;
          case "fakeout":
      ...
      if (abilities[1]) {
        ...
        if (species.name === "Azumarill" && !isDoubles) {
          ...
          if (hasAbility["Harvest"])
            ability = "Harvest";
          if (hasAbility["Healer"] && hasAbility["Natural Cure"])
            ability = "Healer";
    ...
    if (isCloveronly && !fochunItems.includes((0, import_dex.toID)(item))) {
      item = "Leftovers";
    }
    const singlesLevel = randomBattleSet.level || species.randomBattleLevel;
    let level = (!isDoubles ? singlesLevel : species.randomDoubleBattleLevel) || 80;
    if (["shadowtag", "arenatrap", "moody", "wonderguard"].includes((0, import_dex.toID)(ability))) {
      level -= 8;
    }
  ...
  getFestivePokemonPool(pokemon = []) {
    ...
    for (const id in this.dex.data.FormatsData) {
      ...
      if (species.gen > this.gen || exclude.includes(species.id))
        continue;
      if (!species.types.includes("Fairy") && !species.types.includes("Grass") && !species.types.includes("Ice") && !species.types.includes("Normal"))
        continue;
      if (species.num <= 69e3 || species.num > 69386)
        continue;
      pokemonPool.push(id);
    }
    return pokemonPool;
  ...
  getIrishPokemonPool(pokemon = []) {
    ...
    for (const id in this.dex.data.FormatsData) {
      ...
      if (species.gen > this.gen || exclude.includes(species.id))
        continue;
      if ((0, import_dex.toID)(species.color) !== "green")
        continue;
      if (species.num <= 69e3 || species.num > 69386)
        continue;
      pokemonPool.push(id);
  ...
  getSpookyPokemonPool(isCloveronly, pokemon = []) {
    const exclude = pokemon.map((p) => (0, import_dex.toID)(p.species));
    const pokemonPool = [];
    for (const id in this.dex.data.FormatsData) {
      if (["shedinja", "sprucifix"].includes(id))
        continue;
      const species = this.dex.species.get(id);
      if (species.gen > this.gen || exclude.includes(species.id))
        continue;
      if (!species.types.includes("Ghost") && !species.types.includes("Dark"))
        continue;
      if (isCloveronly) {
        if (species.num <= 69e3 || species.num > 69386)
          continue;
  ...
  randomSpookyTeam() {
    ...
    if (pokemon.length + clovermon.length < this.maxTeamSize)
      throw new Error(`Could not build a random team for ${this.format} (seed=${seed})`);
    return [...pokemon, ...clovermon];
  }
  getPokemonPool(type, pokemon = [], isMonotype = false, legalInFormat = false) {
    const exclude = pokemon.map((p) => (0, import_dex.toID)(p.species));
    const pokemonPool = [];
    for (const id in this.dex.data.FormatsData) {
      ...
      if (species.gen > this.gen || exclude.includes(species.id))
        continue;
      if (legalInFormat && species.isNonstandard !== null)
        continue;
      if (isMonotype) {
