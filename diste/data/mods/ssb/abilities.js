...
function changeSet(context, pokemon, newSet, changeAbility = false) {
  ...
  const details = pokemon.species.name + (pokemon.level === 120 ? "" : ", L" + pokemon.level) + (pokemon.gender === "" ? "" : ", " + pokemon.gender) + (pokemon.set.shiny ? ", shiny" : "");
  if (oldShiny !== pokemon.set.shiny)
    context.add("replace", pokemon, details);
  if (changeAbility)
    pokemon.setAbility(newSet.ability);
  ...
  */
  ...
  scyphozoa: {
    ...
    onSwitchIn(source) {
      ...
      for (const pokemon of this.getAllActive()) {
        ...
        if (boostTotal !== 0 || pokemon.positiveBoosts())
          successes++;
        pokemon.clearBoosts();
      ...
      for (const clear in this.field.pseudoWeather) {
        if (clear.endsWith("mod") || clear.endsWith("clause"))
          continue;
  ...
  setthestage: {
    ...
    onSwitchIn(pokemon) {
      ...
      changeSet(this, pokemon, import_random_teams.ssbSets[forme]);
    ...
    onModifyMove(move, attacker, defender) {
      if (attacker.species.baseSpecies !== "Aegislash" || attacker.transformed)
        return;
      if (move.category === "Status" && move.id !== "kingsshield" && move.id !== "reset")
        return;
  ...
  iceage: {
    ...
    onAnySetWeather(target, source, weather) {
      if (this.field.getWeather().id === "heavyhailstorm" && !STRONG_WEATHERS.includes(weather.id))
        return false;
    },
    onEnd(pokemon) {
  ...
  turbulence: {
    ...
    onAnySetWeather(target, source, weather) {
      if (this.field.getWeather().id === "turbulence" && !STRONG_WEATHERS.includes(weather.id))
        return false;
    },
    onEnd(pokemon) {
      if (this.field.weatherState.source !== pokemon)
  ...
  bipolar: {
    ...
    flags: { cantsuppress: 1 },
    onSwitchIn(pokemon) {
      if (pokemon.species.baseSpecies !== "Kartana")
        return;
      const typeMap = {
        Normal: "Return",
  ...
  windingsong: {
    ...
    flags: { cantsuppress: 1 },
    onResidual(pokemon) {
      if (pokemon.species.baseSpecies !== "Alcremie")
        return;
      let coolMoves = [];
  ...
  iceshield: {
    ...
    onTakeItem(item, pokemon, source) {
      if (this.suppressingAbility(pokemon) || !pokemon.hp || pokemon.item === "stickybarb")
        return;
      if (!this.activeMove)
  ...
  armortime: {
    ...
    onBeforeMove(source, target, move) {
      if (move.category !== "Status" && move.id !== "kinggirigirislash")
        return;
  ...
  ic3peak: {
    ...
    onModifyMove(move, attacker) {
      ...
      move.basePowerCallback = function(pokemon, target, move2) {
        if (this.field.pseudoWeather.echoedvoiceclone) {
          if (this.field.pseudoWeather.echoedvoiceclone.lastmove === move2.name) {
            return move2.basePower * this.field.pseudoWeather.echoedvoiceclone.multiplier;
          } else {
  ...
  hydrostatic: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (!["Water", "Electric"].includes(move.type) || move.flags["pledgecombo"])
        return;
  ...
  stubbornness: {
    ...
    onSwitchOut(pokemon) {
      if (this.effectState.happened)
        delete this.effectState.happened;
    },
  ...
  hecatomb: {
    ...
    onSourceAfterFaint(length, target, source, effect) {
      if (effect && effect.effectType === "Move") {
        ...
        if (source.species.baseSpecies !== "Chandelure")
          return;
        if (source.set.shiny)
          return;
        source.m.nowShiny = true;
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${(0, import_conditions.getName)("PartMan")}|THE LIGHT! IT BURNS!`);
        changeSet(this, source, import_random_teams.ssbSets["PartMan-Shiny"]);
      }
    },
    gen: 8
  },
  ...
  foreverwinternights: {
    ...
    onAnySetWeather(target, source, weather) {
      if (this.field.getWeather().id === "winterhail" && !STRONG_WEATHERS.includes(weather.id))
        return false;
    },
    onEnd(pokemon) {
      if (this.field.weatherState.source !== pokemon)
  ...
  thenumbersgame: {
    ...
    flags: { cantsuppress: 1 },
    onStart(target) {
      if (target.baseSpecies.baseSpecies !== "Necrozma" || target.transformed)
        return;
      if (target.side.pokemonLeft <= 3) {
        if (target.species.name === "Necrozma-Dusk-Mane" && target.side.pokemonLeft === 1 && target.m.flag2) {
          changeSet(this, target, import_random_teams.ssbSets["Robb576-Ultra"]);
        } else if (target.species.name === "Necrozma-Dawn-Wings" && target.m.flag1) {
          changeSet(this, target, import_random_teams.ssbSets["Robb576-Dusk-Mane"]);
          target.m.flag2 = true;
        }
      }
      target.m.flag1 = true;
  ...
  shadydeal: {
    ...
    onStart(pokemon) {
      ...
      if (this.effectState.immunities)
        return;
  ...
  combattraining: {
    ...
    onDamage(damage, target, source, effect) {
      ...
      if (effect?.effectType === "Move" && cosplayFormes.includes(target.species.id) && !target.transformed && !this.effectState.busted) {
  ...
  illusion: {
    ...
    onEnd(pokemon) {
      if (pokemon.illusion) {
        ...
        if (this.dex.species.get(disguisedAs).exists)
          disguisedAs += "user";
  ...
  icebody: {
    ...
    onImmunity(type, pokemon) {
      if (["heavyhailstorm", "hail", "winterhail"].includes(type))
        return false;
    }
  },
  iceface: {
    ...
    onStart(pokemon) {
      if (this.field.isWeather(["heavyhailstorm", "hail", "winterhail"]) && pokemon.species.id === "eiscuenoice" && !pokemon.transformed) {
