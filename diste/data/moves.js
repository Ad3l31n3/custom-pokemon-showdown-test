...
const Moves = {
  ...
  allyswitch: {
    ...
    onPrepareHit(pokemon2) {
      return pokemon2.addVolatile("allyswitch");
    },
    onHit(pokemon2) {
      let success = true;
      if (this.format.gameType !== "doubles" && this.format.gameType !== "triples")
        success = false;
      if (pokemon2.side.active.length === 3 && pokemon2.position === 1)
        success = false;
      const newPosition = pokemon2.position === 0 ? pokemon2.side.active.length - 1 : 0;
      if (!pokemon2.side.active[newPosition])
        success = false;
      if (pokemon2.side.active[newPosition].fainted)
        success = false;
      if (!success) {
        this.add("-fail", pokemon2, "move: Ally Switch");
        this.attrLastMove("[still]");
        return this.NOT_FAIL;
      }
      this.swapPosition(pokemon2, newPosition, "[from] move: Ally Switch");
    },
    condition: {
      duration: 2,
      counterMax: 729,
      ...
      onRestart(pokemon2) {
        ...
        if (!success) {
          delete pokemon2.volatiles["allyswitch"];
          return false;
        }
  ...
  aquaring: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Aqua Ring");
      },
      onResidualOrder: 6,
      onResidual(pokemon2) {
  ...
  assist: {
    ...
    onHit(target) {
      const moves = [];
      for (const pokemon2 of target.side.pokemon) {
        if (pokemon2 === target)
          continue;
  ...
  aurawheel: {
    ...
    onModifyType(move, pokemon2) {
      if (pokemon2.species.name === "Morpeko-Hangry") {
        move.type = "Dark";
      } else {
        move.type = "Electric";
      }
  ...
  autotomize: {
    ...
    flags: { snatch: 1, metronome: 1 },
    onTryHit(pokemon2) {
      const hasContrary = pokemon2.hasAbility("contrary");
      if (!hasContrary && pokemon2.boosts.spe === 6 || hasContrary && pokemon2.boosts.spe === -6) {
        return false;
      }
    },
    boosts: {
    ...
    onHit(pokemon2) {
      if (pokemon2.weighthg > 1) {
        pokemon2.weighthg = Math.max(1, pokemon2.weighthg - 1e3);
        this.add("-start", pokemon2, "Autotomize");
      }
    },
    secondary: null,
    target: "self",
  ...
  beakblast: {
    ...
    priorityChargeCallback(pokemon2) {
      pokemon2.addVolatile("beakblast");
    },
    condition: {
      duration: 1,
      onStart(pokemon2) {
        this.add("-singleturn", pokemon2, "move: Beak Blast");
      },
      onHit(target, source, move) {
        if (this.checkMoveMakesContact(move, source, target)) {
    ...
    onAfterMove(pokemon2) {
      pokemon2.removeVolatile("beakblast");
    },
    secondary: null,
    target: "normal",
    type: "Flying",
  ...
  beatup: {
    ...
    onModifyMove(move, pokemon2) {
      move.allies = pokemon2.side.pokemon.filter((ally) => ally === pokemon2 || !ally.fainted && !ally.status);
  ...
  belch: {
    ...
    onDisableMove(pokemon2) {
      if (!pokemon2.ateBerry)
        pokemon2.disableMove("belch");
    },
    secondary: null,
    target: "normal",
    type: "Poison",
  ...
  bide: {
    ...
    condition: {
      ...
      onStart(pokemon2) {
        ...
        this.add("-start", pokemon2, "move: Bide");
      },
      ...
      onBeforeMove(pokemon2, target, move) {
        if (this.effectState.duration === 1) {
          this.add("-end", pokemon2, "move: Bide");
          ...
          if (!target.isActive) {
            const possibleTarget = this.getRandomTarget(pokemon2, this.dex.moves.get("pound"));
            if (!possibleTarget) {
              this.add("-miss", pokemon2);
              return false;
            }
          ...
          this.actions.tryMoveHit(target, pokemon2, moveData);
          pokemon2.removeVolatile("bide");
          return false;
        }
        this.add("-activate", pokemon2, "move: Bide");
      },
      onMoveAborted(pokemon2) {
        pokemon2.removeVolatile("bide");
      },
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "move: Bide", "[silent]");
      }
    },
    secondary: null,
    target: "self",
  ...
  burnup: {
    ...
    onTryMove(pokemon2, target, move) {
      if (pokemon2.hasType("Fire"))
        return;
      this.add("-fail", pokemon2, "move: Burn Up");
      this.attrLastMove("[still]");
      return null;
    },
    self: {
      onHit(pokemon2) {
        pokemon2.setType(pokemon2.getTypes(true).map((type) => type === "Fire" ? "???" : type));
        this.add("-start", pokemon2, "typechange", pokemon2.getTypes().join("/"), "[from] move: Burn Up");
      }
    },
    secondary: null,
    target: "normal",
  ...
  captivate: {
    ...
    onTryImmunity(pokemon2, source) {
      return pokemon2.gender === "M" && source.gender === "F" || pokemon2.gender === "F" && source.gender === "M";
    },
    boosts: {
      spa: -2
    },
    ...
    condition: {
      onStart(pokemon2, source, effect) {
        if (effect && ["Electromorphosis", "Wind Power"].includes(effect.name)) {
          this.add("-start", pokemon2, "Charge", this.activeMove.name, "[from] ability: " + effect.name);
        } else {
          this.add("-start", pokemon2, "Charge");
        }
      },
      onRestart(pokemon2, source, effect) {
        if (effect && ["Electromorphosis", "Wind Power"].includes(effect.name)) {
          this.add("-start", pokemon2, "Charge", this.activeMove.name, "[from] ability: " + effect.name);
        } else {
          this.add("-start", pokemon2, "Charge");
      ...
      onMoveAborted(pokemon2, target, move) {
        if (move.type === "Electric" && move.id !== "charge") {
          pokemon2.removeVolatile("charge");
        }
      },
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Charge", "[silent]");
      }
    },
    boosts: {
      spd: 1
  ...
  clangoroussoul: {
    ...
    onHit(pokemon2) {
      this.directDamage(pokemon2.maxhp * 33 / 100);
    },
    boosts: {
      atk: 1,
      def: 1,
  ...
  comeuppance: {
    ...
    accuracy: 100,
    basePower: 0,
    damageCallback(pokemon2) {
      const lastDamagedBy = pokemon2.getLastDamagedBy(true);
  ...
  copycat: {
    ...
    onHit(pokemon2) {
      ...
      this.actions.useMove(move.id, pokemon2);
    },
    secondary: null,
    target: "self",
    type: "Normal",
  ...
  counter: {
    ...
    basePower: 0,
    damageCallback(pokemon2) {
      if (!pokemon2.volatiles["counter"])
        return 0;
      return pokemon2.volatiles["counter"].damage || 1;
    },
    category: "Physical",
    name: "Counter",
    pp: 20,
  ...
  curse: {
    ...
    condition: {
      onStart(pokemon2, source) {
        this.add("-start", pokemon2, "Curse", "[of] " + source);
      },
      onResidualOrder: 12,
      onResidual(pokemon2) {
  ...
  destinybond: {
    ...
    onPrepareHit(pokemon2) {
      return !pokemon2.removeVolatile("destinybond");
    },
    condition: {
      onStart(pokemon2) {
        this.add("-singlemove", pokemon2, "Destiny Bond");
      },
  ...
  disable: {
    ...
    condition: {
      ...
      // doesn't get copied by Baton Pass
      onStart(pokemon2, source, effect) {
        if (this.queue.willMove(pokemon2) || pokemon2 === this.activePokemon && this.activeMove && !this.activeMove.isExternal) {
          this.effectState.duration--;
        }
        if (!pokemon2.lastMove) {
          this.debug(`Pokemon hasn't moved yet`);
          return false;
        }
        for (const moveSlot of pokemon2.moveSlots) {
          if (moveSlot.id === pokemon2.lastMove.id) {
            if (!moveSlot.pp) {
        ...
        if (effect.effectType === "Ability") {
          this.add("-start", pokemon2, "Disable", pokemon2.lastMove.name, "[from] ability: Cursed Body", "[of] " + source);
        } else {
          this.add("-start", pokemon2, "Disable", pokemon2.lastMove.name);
        }
        this.effectState.move = pokemon2.lastMove.id;
      },
      onResidualOrder: 17,
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Disable");
      },
  ...
  doodle: {
    ...
    onHit(target, source, move) {
      ...
      if (!target.getAbility().flags["failroleplay"]) {
        for (const pokemon2 of source.alliesAndSelf()) {
          if (pokemon2.ability === target.ability || pokemon2.getAbility().flags["cantsuppress"])
            continue;
          const oldAbility = pokemon2.setAbility(target.ability);
          if (oldAbility) {
            this.add("-ability", pokemon2, target.getAbility().name, "[from] move: Doodle");
            success = true;
  ...
  doubleshock: {
    ...
    flags: { contact: 1, protect: 1, mirror: 1 },
    onTryMove(pokemon2, target, move) {
      if (pokemon2.hasType("Electric"))
        return;
      this.add("-fail", pokemon2, "move: Double Shock");
      this.attrLastMove("[still]");
      return null;
    },
    self: {
      onHit(pokemon2) {
        pokemon2.setType(pokemon2.getTypes(true).map((type) => type === "Electric" ? "???" : type));
        this.add("-start", pokemon2, "typechange", pokemon2.getTypes().join("/"), "[from] move: Double Shock");
      }
    },
    secondary: null,
    target: "normal",
  ...
  dragonenergy: {
    ...
    basePower: 150,
    basePowerCallback(pokemon2, target, move) {
      const bp = move.basePower * pokemon2.hp / pokemon2.maxhp;
      this.debug("BP: " + bp);
      return bp;
    },
    category: "Special",
  ...
  embargo: {
    ...
    condition: {
      duration: 5,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Embargo");
        this.singleEvent("End", pokemon2.getItem(), pokemon2.itemState, pokemon2);
      },
      // Item suppression implemented in Pokemon.ignoringItem() within sim/pokemon.js
      onResidualOrder: 21,
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Embargo");
      }
    },
    secondary: null,
    target: "normal",
  ...
  encore: {
    ...
    condition: {
      ...
      onOverrideAction(pokemon2, target, move) {
      ...
      onDisableMove(pokemon2) {
        if (!this.effectState.move || !pokemon2.hasMove(this.effectState.move)) {
          return;
        }
        for (const moveSlot of pokemon2.moveSlots) {
          if (moveSlot.id !== this.effectState.move) {
  ...
  endeavor: {
    ...
    accuracy: 100,
    basePower: 0,
    damageCallback(pokemon2, target) {
      return target.getUndynamaxedHP() - pokemon2.hp;
    },
    category: "Physical",
    name: "Endeavor",
    pp: 5,
    ...
    onTryImmunity(target, pokemon2) {
      return pokemon2.hp < target.hp;
    },
    secondary: null,
    target: "normal",
    type: "Normal",
  ...
  facade: {
    ...
    onBasePower(basePower, pokemon2) {
      if (pokemon2.status && pokemon2.status !== "slp") {
        return this.chainModify(2);
      }
    },
    secondary: null,
  ...
  fellstinger: {
    ...
    onAfterMoveSecondarySelf(pokemon2, target, move) {
      if (!target || target.fainted || target.hp <= 0)
        this.boost({ atk: 3 }, pokemon2, pokemon2, move);
    },
    secondary: null,
    target: "normal",
    type: "Bug",
  ...
  ficklebeam: {
    ...
    onBasePower(basePower, pokemon2) {
      if (this.randomChance(3, 10)) {
        ...
        this.add("-activate", pokemon2, "move: Fickle Beam");
        return this.chainModify(2);
      }
    },
    secondary: null,
  ...
  filletaway: {
    ...
    onHit(pokemon2) {
      this.directDamage(pokemon2.maxhp / 2);
    },
    boosts: {
      atk: 2,
      spa: 2,
  ...
  firepledge: {
    ...
    condition: {
      ...
      onResidual(pokemon2) {
        if (!pokemon2.hasType("Fire"))
          this.damage(pokemon2.baseMaxhp / 8, pokemon2);
      },
      onSideResidualOrder: 26,
  ...
  flail: {
    ...
    basePower: 0,
    basePowerCallback(pokemon2, target) {
      const ratio = Math.max(Math.floor(pokemon2.hp * 48 / pokemon2.maxhp), 1);
      let bp;
      if (ratio < 2) {
        bp = 200;
      } else if (ratio < 5) {
  ...
  fling: {
    ...
    condition: {
      onUpdate(pokemon2) {
        ...
        this.add("-enditem", pokemon2, item.name, "[from] move: Fling");
  ...
  flowershield: {
    ...
    onHitField(t, source, move) {
      ...
      for (const pokemon2 of this.getAllActive()) {
        if (pokemon2.hasType("Grass") && (!pokemon2.volatiles["maxguard"] || this.runEvent("TryHit", pokemon2, source, move))) {
          targets.push(pokemon2);
        }
      }
      let success = false;
  ...
  focuspunch: {
    ...
    priorityChargeCallback(pokemon2) {
      pokemon2.addVolatile("focuspunch");
    },
    beforeMoveCallback(pokemon2) {
      if (pokemon2.volatiles["focuspunch"]?.lostFocus) {
        this.add("cant", pokemon2, "Focus Punch", "Focus Punch");
        return true;
      }
    },
    condition: {
      duration: 1,
      onStart(pokemon2) {
        this.add("-singleturn", pokemon2, "move: Focus Punch");
      },
  ...
  foresight: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Foresight");
      },
      onNegateImmunity(pokemon2, type) {
        if (pokemon2.hasType("Ghost") && ["Normal", "Fighting"].includes(type))
          return false;
      },
  ...
  forestscurse: {
    ...
    onHit(target) {
      ...
      if (!target.addType("Grass"))
        return false;
      if (target.hasAbility("etherealshroud") || target.hasAbility("omnitype"))
        return false;
  ...
  frustration: {
    ...
    basePowerCallback(pokemon2) {
      return Math.floor((255 - pokemon2.happiness) * 10 / 25) || 1;
    },
    category: "Physical",
    isNonstandard: "PastMove",
    name: "Frustration",
  ...
  furycutter: {
    ...
    basePower: 40,
    basePowerCallback(pokemon2, target, move) {
      if (!pokemon2.volatiles["furycutter"] || move.hit === 1) {
        pokemon2.addVolatile("furycutter");
      }
      const bp = this.clampIntRange(move.basePower * pokemon2.volatiles["furycutter"].multiplier, 1, 160);
      this.debug("BP: " + bp);
      return bp;
    },
    category: "Physical",
    ...
    condition: {
      ...
      onStart(target) {
        ...
        if (target.illusion) {
          ...
          const details = target.species.name + (target.level === 120 ? "" : ", L" + target.level) + (target.gender === "" ? "" : ", " + target.gender) + (target.set.shiny ? ", shiny" : "");
  ...
  glaiverush: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon2) {
        this.add("-singlemove", pokemon2, "Glaive Rush", "[silent]");
      },
      onAccuracy() {
        return true;
      },
      ...
      onBeforeMove(pokemon2) {
        ...
        pokemon2.removeVolatile("glaiverush");
      }
    },
    secondary: null,
    target: "normal",
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.foes()) {
          const result = this.random(3);
          if (result === 0) {
            pokemon2.trySetStatus("slp", source);
          } else if (result === 1) {
            pokemon2.trySetStatus("par", source);
          } else {
            if (source.hasAbility("venomous")) {
              pokemon2.trySetStatus("tox", source);
            } else {
              pokemon2.trySetStatus("psn", source);
            }
          }
        }
      }
  ...
  gmaxcentiferno: {
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.foes()) {
          pokemon2.addVolatile("partiallytrapped", source, this.dex.getActiveMove("G-Max Centiferno"));
        }
      }
    },
    secondary: null,
  ...
  gmaxchistrike: {
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.alliesAndSelf()) {
          pokemon2.addVolatile("gmaxchistrike");
        }
      }
    },
    condition: {
  ...
  gmaxdepletion: {
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.foes()) {
          let move = pokemon2.lastMove;
          if (!move || move.isZ)
            continue;
          if (move.isMax && move.baseMove)
            move = this.dex.moves.get(move.baseMove);
          const ppDeducted = pokemon2.deductPP(move.id, 2);
          if (ppDeducted) {
            this.add("-activate", pokemon2, "move: G-Max Depletion", move.name, ppDeducted);
          }
        }
      }
    },
  ...
  gmaxfinale: {
    ...
    self: {
      onHit(target, source, move) {
        for (const pokemon2 of source.alliesAndSelf()) {
          this.heal(pokemon2.maxhp / 6, pokemon2, source, move);
        }
      }
    },
    secondary: null,
      ...
      onHit(source, target, effect) {
        for (const pokemon2 of source.foes()) {
          if (!pokemon2.volatiles["dynamax"])
            pokemon2.addVolatile("torment", source, effect);
        }
      }
    },
    secondary: null,
    ...
    self: {
      onHit(source) {
        ...
        for (const pokemon2 of source.alliesAndSelf()) {
          if (pokemon2.item)
            continue;
          if (pokemon2.lastItem && this.dex.items.get(pokemon2.lastItem).isBerry) {
            const item = pokemon2.lastItem;
            pokemon2.lastItem = "";
            this.add("-item", pokemon2, this.dex.items.get(item), "[from] move: G-Max Replenish");
  ...
  gmaxsandblast: {
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.foes()) {
          pokemon2.addVolatile("partiallytrapped", source, this.dex.getActiveMove("G-Max Sandblast"));
        }
      }
    },
    secondary: null,
  ...
  gmaxsmite: {
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.foes()) {
          pokemon2.addVolatile("confusion", source);
        }
      }
    },
    secondary: null,
  ...
  gmaxterror: {
    ...
    self: {
      onHit(source) {
        for (const pokemon2 of source.foes()) {
          pokemon2.addVolatile("trapped", source, null, "trapper");
        }
      }
    },
    secondary: null,
  ...
  grassyterrain: {
    ...
    condition: {
      ...
      onResidual(pokemon2) {
        if (pokemon2.isGrounded() && !pokemon2.isSemiInvulnerable()) {
  ...
  gravity: {
    ...
    condition: {
      ...
      onFieldStart(target, source) {
        ...
        for (const pokemon2 of this.getAllActive()) {
          let applies = false;
          if (pokemon2.removeVolatile("bounce") || pokemon2.removeVolatile("fly")) {
            applies = true;
            this.queue.cancelMove(pokemon2);
            pokemon2.removeVolatile("twoturnmove");
          }
          if (pokemon2.volatiles["skydrop"]) {
            applies = true;
            this.queue.cancelMove(pokemon2);
            if (pokemon2.volatiles["skydrop"].source) {
              this.add("-end", pokemon2.volatiles["twoturnmove"].source, "Sky Drop", "[interrupt]");
            }
            pokemon2.removeVolatile("skydrop");
            pokemon2.removeVolatile("twoturnmove");
          }
          ...
          if (applies)
            this.add("-activate", pokemon2, "move: Gravity");
        }
      },
      ...
      onBeforeMove(pokemon2, target, move) {
        if (move.flags["gravity"] && !move.isZ) {
          this.add("cant", pokemon2, "move: Gravity", move);
          return false;
        }
      },
      onModifyMove(move, pokemon2, target) {
  ...
  gyroball: {
    ...
    basePower: 0,
    basePowerCallback(pokemon2, target) {
      let power = Math.floor(25 * target.getStat("spe") / pokemon2.getStat("spe")) + 1;
      if (!isFinite(power))
        power = 1;
      if (power > 150)
        power = 150;
  ...
  heatcrash: {
    ...
    basePowerCallback(pokemon2, target) {
      const targetWeight = target.getWeight();
      const pokemonWeight = pokemon2.getWeight();
      let bp;
    ...
    priority: 0,
    flags: { contact: 1, protect: 1, mirror: 1, nonsky: 1, metronome: 1 },
    onTryHit(target, pokemon2, move) {
      if (target.volatiles["dynamax"]) {
        this.add("-fail", pokemon2, "Dynamax");
        this.attrLastMove("[still]");
        return null;
      }
    },
  ...
  hiddenpower: {
    ...
    flags: { protect: 1, mirror: 1, metronome: 1 },
    onModifyType(move, pokemon2) {
      move.type = pokemon2.hpType || "Dark";
    },
    secondary: null,
    target: "normal",
    type: "Normal",
  ...
  iceball: {
    ...
    onModifyMove(move, pokemon2, target) {
      if (pokemon2.volatiles["iceball"] || pokemon2.status === "slp" || !target)
        return;
      pokemon2.addVolatile("iceball");
      pokemon2.volatiles["iceball"].targetSlot = move.sourceEffect ? pokemon2.lastMoveTargetLoc : pokemon2.getLocOf(target);
    },
  ...
  imprison: {
    ...
    condition: {
      ...
      onFoeDisableMove(pokemon2) {
        for (const moveSlot of this.effectState.source.moveSlots) {
          if (moveSlot.id === "struggle")
            continue;
          pokemon2.disableMove(moveSlot.id, "hidden");
        }
        pokemon2.maybeDisabled = true;
      },
  ...
  incinerate: {
    ...
    priority: 0,
    flags: { protect: 1, mirror: 1, metronome: 1 },
    onHit(pokemon2, source) {
      const item = pokemon2.getItem();
      if ((item.isBerry || item.isGem) && pokemon2.takeItem(source)) {
        this.add("-enditem", pokemon2, item.name, "[from] move: Incinerate");
      }
    },
    secondary: null,
    target: "allAdjacentFoes",
  ...
  ingrain: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-start", pokemon2, "move: Ingrain");
      },
      onResidualOrder: 7,
      onResidual(pokemon2) {
        this.heal(pokemon2.baseMaxhp / 16);
      ...
      // groundedness implemented in battle.engine.js:BattlePokemon#isGrounded
      onDragOut(pokemon2) {
        this.add("-activate", pokemon2, "move: Ingrain");
        return null;
      }
    },
    secondary: null,
  ...
  junglehealing: {
    ...
    onHit(pokemon2) {
      const success = !!this.heal(this.modify(pokemon2.maxhp, 0.25));
  ...
  laserfocus: {
    ...
    condition: {
      ...
      onStart(pokemon2, source, effect) {
        if (effect && ["costar", "imposter", "psychup", "transform"].includes(effect.id)) {
          this.add("-start", pokemon2, "move: Laser Focus", "[silent]");
        } else {
          this.add("-start", pokemon2, "move: Laser Focus");
        }
      },
      onRestart(pokemon2) {
        this.effectState.duration = 2;
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "move: Laser Focus", "[silent]");
      }
    },
    secondary: null,
    target: "self",
  ...
  lastrespects: {
    ...
    basePower: 50,
    basePowerCallback(pokemon2, target, move) {
      return 50 + 50 * pokemon2.side.totalFainted;
    },
    category: "Physical",
    name: "Last Respects",
    pp: 10,
  ...
  leechseed: {
    ...
    condition: {
      ...
      onResidualOrder: 8,
      onResidual(pokemon2) {
        const target = this.getAtSlot(pokemon2.volatiles["leechseed"].sourceSlot);
        if (!target || target.fainted || target.hp <= 0) {
          this.debug("Nothing to leech into");
          return;
        }
        const damage = this.damage(pokemon2.baseMaxhp / 8, pokemon2, target);
        if (damage) {
    ...
    onTryImmunity(target, source, move) {
      if (move.ignoreAbility && !target.hasType("Grass"))
        return true;
    ...
    condition: {
      ...
      onSwitchIn(pokemon2) {
        ...
        if (!pokemon2.isGrounded())
          return;
        if (pokemon2.hasType("Electric") || pokemon2.hasType("Ground")) {
          this.add("-sideend", pokemon2.side, "move: Livewire", "[of] " + pokemon2);
          pokemon2.side.removeSideCondition("livewire");
        } else if (pokemon2.hasItem("heavydutyboots")) {
          return;
        } else if (["raindance", "primordialsea"].includes(pokemon2.effectiveWeather()) && 4 * this.effectState.layers >= rand) {
          pokemon2.trySetStatus("par", pokemon2.side.foe.active[0]);
  ...
  magnitude: {
    ...
    onUseMoveMessage(pokemon2, target, move) {
      this.add("-activate", pokemon2, "move: Magnitude", move.magnitude);
    },
    secondary: null,
    target: "allAdjacent",
    type: "Ground",
  ...
  mindblown: {
    ...
    onAfterMove(pokemon2, target, move) {
      if (move.mindBlownRecoil && !move.multihit) {
        const hpBeforeRecoil = pokemon2.hp;
        this.damage(Math.round(pokemon2.maxhp / 2), pokemon2, pokemon2, this.dex.conditions.get("Mind Blown"), true);
        if (pokemon2.hp <= pokemon2.maxhp / 2 && hpBeforeRecoil > pokemon2.maxhp / 2) {
          this.runEvent("EmergencyExit", pokemon2, pokemon2);
        }
      }
    },
    secondary: null,
  ...
  miracleeye: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Miracle Eye");
      },
      onNegateImmunity(pokemon2, type) {
        if (pokemon2.hasType("Dark") && type === "Psychic")
          return false;
      },
      onModifyBoost(boosts) {
        if (boosts.evasion && boosts.evasion > 0) {
  ...
  mirrorcoat: {
    ...
    basePower: 0,
    damageCallback(pokemon2) {
      if (!pokemon2.volatiles["mirrorcoat"])
        return 0;
      return pokemon2.volatiles["mirrorcoat"].damage || 1;
    },
    category: "Special",
    name: "Mirror Coat",
    pp: 20,
    ...
    beforeTurnCallback(pokemon2) {
      pokemon2.addVolatile("mirrorcoat");
    },
    onTry(source) {
  ...
  mirrormove: {
    ...
    onTryHit(target, pokemon2) {
      ...
      this.actions.useMove(move.id, pokemon2, target);
      return null;
    },
    secondary: null,
    target: "normal",
  ...
  morph: {
    ...
    priority: 0,
    flags: { allyanim: 1 },
    onHit(target, pokemon2) {
      if (!pokemon2.morphInto(target)) {
        return false;
      }
    },
    secondary: null,
    ...
    onAfterHit(target, pokemon2, move) {
      if (!move.hasSheerForce) {
        if (pokemon2.hp && pokemon2.removeVolatile("leechseed")) {
          this.add("-end", pokemon2, "Leech Seed", "[from] move: Mortal Spin", "[of] " + pokemon2);
        }
        ...
        for (const condition of sideConditions) {
          if (pokemon2.hp && pokemon2.side.removeSideCondition(condition)) {
            this.add("-sideend", pokemon2.side, this.dex.conditions.get(condition).name, "[from] move: Mortal Spin", "[of] " + pokemon2);
          }
        }
  ...
  multiattack: {
    ...
    onModifyType(move, pokemon2) {
      if (pokemon2.ignoringItem())
        return;
      move.type = this.runEvent("Memory", pokemon2, null, move, "Normal");
    },
    secondary: null,
    target: "normal",
    type: "Normal",
  ...
  nightmare: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon2) {
        if (pokemon2.status !== "slp" && !pokemon2.hasAbility("comatose")) {
          return false;
        }
      ...
      onResidual(pokemon2) {
        if (this.field.isWeather("newmoon")) {
          this.damage(pokemon2.baseMaxhp / 2);
        } else {
          this.damage(pokemon2.baseMaxhp / 4);
        }
      }
  ...
  noretreat: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-start", pokemon2, "move: No Retreat");
      },
      onTrapPokemon(pokemon2) {
        pokemon2.tryTrap();
      }
  ...
  octolock: {
    ...
    condition: {
      onStart(pokemon2, source) {
        this.add("-start", pokemon2, "move: Octolock", "[of] " + source);
      },
      onResidualOrder: 14,
      onResidual(pokemon2) {
        const source = this.effectState.source;
        if (source && (!source.isActive || source.hp <= 0 || !source.activeTurns)) {
          delete pokemon2.volatiles["octolock"];
          this.add("-end", pokemon2, "Octolock", "[partiallytrapped]", "[silent]");
          return;
        }
        this.boost({ def: -1, spd: -1 }, pokemon2, source, this.dex.getActiveMove("octolock"));
      },
      onTrapPokemon(pokemon2) {
  ...
  orderup: {
    ...
    flags: { protect: 1 },
    onAfterMoveSecondarySelf(pokemon2, target, move) {
      if (!pokemon2.volatiles["commanded"])
        return;
      const tatsugiri = pokemon2.volatiles["commanded"].source;
      if (tatsugiri.baseSpecies.baseSpecies !== "Tatsugiri")
        return;
      switch (tatsugiri.baseSpecies.forme) {
        case "Droopy":
          this.boost({ def: 1 }, pokemon2, pokemon2);
          break;
        case "Stretchy":
  ...
  outrage: {
    ...
    onAfterMove(pokemon2) {
      if (pokemon2.volatiles["lockedmove"] && pokemon2.volatiles["lockedmove"].duration === 1) {
        pokemon2.removeVolatile("lockedmove");
      }
    },
    secondary: null,
    target: "randomNormal",
  ...
  painsplit: {
    ...
    onHit(target, pokemon2) {
      ...
      this.add("-sethp", pokemon2, pokemon2.getHealth, "[from] move: Pain Split");
    },
    secondary: null,
    target: "normal",
    type: "Normal",
  ...
  permafrost: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon2) {
        ...
        if (!pokemon2.isGrounded())
          return;
        if (pokemon2.hasType("Ice") || pokemon2.hasType("Fire")) {
          this.add("-sideend", pokemon2.side, "move: Permafrost", "[of] " + pokemon2);
          pokemon2.side.removeSideCondition("permafrost");
        } else if (pokemon2.hasItem("heavydutyboots")) {
          return;
        } else if (["hail"].includes(pokemon2.effectiveWeather()) && 2 * this.effectState.layers >= rand) {
          pokemon2.trySetStatus("frz", pokemon2.side.foe.active[0]);
  ...
  pikapapow: {
    ...
    basePower: 0,
    basePowerCallback(pokemon2) {
      const bp = Math.floor(pokemon2.happiness * 10 / 25) || 1;
      this.debug("BP: " + bp);
      return bp;
    },
    category: "Special",
  ...
  powder: {
    ...
    condition: {
      ...
      onTryMove(pokemon2, target, move) {
        if (move.type === "Fire") {
          this.add("-activate", pokemon2, "move: Powder");
          this.damage(this.clampIntRange(Math.round(pokemon2.maxhp / 4), 1));
          this.attrLastMove("[still]");
          return false;
        }
      }
  ...
  powershift: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Power Shift");
        const newatk = pokemon2.storedStats.def;
        const newdef = pokemon2.storedStats.atk;
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Power Shift");
        const newatk = pokemon2.storedStats.def;
        const newdef = pokemon2.storedStats.atk;
        pokemon2.storedStats.atk = newatk;
        pokemon2.storedStats.def = newdef;
      ...
      onRestart(pokemon2) {
        pokemon2.removeVolatile("Power Shift");
      }
    },
    secondary: null,
    target: "self",
  ...
  powertrick: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Power Trick");
        const newatk = pokemon2.storedStats.def;
        const newdef = pokemon2.storedStats.atk;
        pokemon2.storedStats.atk = newatk;
        pokemon2.storedStats.def = newdef;
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Power Trick");
        const newatk = pokemon2.storedStats.def;
        const newdef = pokemon2.storedStats.atk;
        pokemon2.storedStats.atk = newatk;
        pokemon2.storedStats.def = newdef;
      ...
      onRestart(pokemon2) {
        pokemon2.removeVolatile("Power Trick");
      }
    },
    secondary: null,
    target: "self",
  ...
  powertrip: {
    ...
    basePower: 20,
    basePowerCallback(pokemon2, target, move) {
      const bp = move.basePower + 20 * pokemon2.positiveBoosts();
      this.debug("BP: " + bp);
      return bp;
    },
    category: "Physical",
  ...
  psywave: {
    ...
    basePower: 0,
    damageCallback(pokemon2) {
      return this.random(50, 151) * pokemon2.level / 100;
    },
    category: "Special",
    isNonstandard: "PastMove",
    name: "Psywave",
  ...
  pursuit: {
    ...
    beforeTurnCallback(pokemon2) {
      for (const side of this.sides) {
        if (side.hasAlly(pokemon2))
          continue;
        side.addSideCondition("pursuit", pokemon2);
      ...
      onBeforeSwitchOut(pokemon2) {
        ...
        for (const source of this.effectState.sources) {
          if (!source.isAdjacent(pokemon2) || !this.queue.cancelMove(source) || !source.hp)
            continue;
          if (!alreadyAdded) {
            this.add("-activate", pokemon2, "move: Pursuit");
            alreadyAdded = true;
          }
          ...
          this.actions.runMove("pursuit", source, source.getLocOf(pokemon2));
        }
      }
    },
    secondary: null,
  ...
  rage: {
    ...
    condition: {
      onStart(pokemon2) {
        this.add("-singlemove", pokemon2, "Rage");
      },
      onHit(target, source, move) {
  ...
  ragefist: {
    ...
    basePower: 50,
    basePowerCallback(pokemon2) {
      return Math.min(350, 50 + 50 * pokemon2.timesAttacked);
    },
    category: "Physical",
    name: "Rage Fist",
    pp: 10,
  ...
  ragepowder: {
    ...
    condition: {
      duration: 1,
      onStart(pokemon2) {
        this.add("-singleturn", pokemon2, "move: Rage Powder");
      },
      onFoeRedirectTargetPriority: 1,
      onFoeRedirectTarget(target, source, source2, move) {
  ...
  ragingfury: {
    ...
    onAfterMove(pokemon2) {
      if (pokemon2.volatiles["lockedmove"]?.duration === 1) {
        pokemon2.removeVolatile("lockedmove");
      }
    },
    secondary: null,
  ...
  recycle: {
    ...
    flags: { snatch: 1, metronome: 1 },
    onHit(pokemon2) {
      if (pokemon2.item || !pokemon2.lastItem)
        return false;
      const item = pokemon2.lastItem;
      pokemon2.lastItem = "";
      this.add("-item", pokemon2, this.dex.items.get(item), "[from] move: Recycle");
      pokemon2.setItem(item);
    },
    secondary: null,
    target: "self",
  ...
  refresh: {
    ...
    flags: { snatch: 1, metronome: 1 },
    onHit(pokemon2) {
      if (["", "slp", "frz"].includes(pokemon2.status))
        return false;
      pokemon2.cureStatus();
    },
    secondary: null,
  ...
  relicsong: {
    ...
    onHit(target, pokemon2, move) {
      if (pokemon2.baseSpecies.baseSpecies === "Meloetta" && !pokemon2.transformed) {
        move.willChangeForme = true;
      }
      if (pokemon2.baseSpecies.baseSpecies === "Meloetta-Delta-Mime" && !pokemon2.transformed) {
        move.willChangeFormeDelta = true;
      }
    },
    onAfterMoveSecondarySelf(pokemon2, target, move) {
      if (move.willChangeForme) {
        const meloettaForme = pokemon2.species.id === "meloettapirouette" ? "" : "-Pirouette";
        pokemon2.formeChange("Meloetta" + meloettaForme, this.effect, false, "[msg]");
      }
      if (move.willChangeFormeDelta) {
        const meloettaForme = pokemon2.species.id === "meloettadeltamagician" ? "-Mime" : "-Magician";
        pokemon2.formeChange("Meloetta-Delta" + meloettaForme, this.effect, true, "[msg]");
      }
    },
    target: "allAdjacentFoes",
    type: "Normal",
  ...
  return: {
    ...
    basePower: 0,
    basePowerCallback(pokemon2) {
      return Math.floor(pokemon2.happiness * 10 / 25) || 1;
    },
    category: "Physical",
    isNonstandard: "PastMove",
    name: "Return",
  ...
  rollout: {
    ...
    basePowerCallback(pokemon2, target, move) {
      let bp = move.basePower;
      const rolloutData = pokemon2.volatiles["rollout"];
      if (rolloutData?.hitCount) {
        bp *= Math.pow(2, rolloutData.contactHitCount);
      }
      if (rolloutData && pokemon2.status !== "slp") {
    ...
    onModifyMove(move, pokemon2, target) {
      if (pokemon2.volatiles["rollout"] || pokemon2.status === "slp" || !target)
        return;
      pokemon2.addVolatile("rollout");
      pokemon2.volatiles["rollout"].targetSlot = move.sourceEffect ? pokemon2.lastMoveTargetLoc : pokemon2.getLocOf(target);
    },
    onAfterMove(source, target, move) {
  ...
  rototiller: {
    ...
    onHitField(target, source) {
      ...
      for (const pokemon2 of this.getAllActive()) {
        if (!pokemon2.runImmunity("Ground")) {
          this.add("-immune", pokemon2);
          anyAirborne = true;
          continue;
        }
        if (pokemon2.hasType("Grass")) {
          targets.push(pokemon2);
        }
      }
      ...
      for (const pokemon2 of targets) {
        this.boost({ atk: 1, spa: 1 }, pokemon2, source);
      }
    },
    secondary: null,
    target: "all",
  ...
  saltcure: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Salt Cure");
      },
      onResidualOrder: 13,
      onResidual(pokemon2) {
        this.damage(pokemon2.baseMaxhp / (pokemon2.hasType(["Water", "Steel"]) ? 4 : 8));
      },
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Salt Cure");
      }
    },
    secondary: {
      chance: 100,
  ...
  shelltrap: {
    ...
    priorityChargeCallback(pokemon2) {
      pokemon2.addVolatile("shelltrap");
    },
    onTryMove(pokemon2) {
      if (!pokemon2.volatiles["shelltrap"]?.gotHit) {
        this.attrLastMove("[still]");
        this.add("cant", pokemon2, "Shell Trap", "Shell Trap");
        return null;
      }
    },
    condition: {
      duration: 1,
      onStart(pokemon2) {
        this.add("-singleturn", pokemon2, "move: Shell Trap");
      },
      onHit(pokemon2, source, move) {
        if (!pokemon2.isAlly(source) && move.category === "Physical") {
  ...
  simplebeam: {
    ...
    onHit(pokemon2) {
      const oldAbility = pokemon2.setAbility("simple");
      if (oldAbility) {
        this.add("-ability", pokemon2, "Simple", "[from] move: Simple Beam");
        return;
      }
      return oldAbility;
    },
  ...
  sleeptalk: {
    ...
    onHit(pokemon2) {
      ...
      this.actions.useMove(randomMove, pokemon2);
    },
    secondary: null,
    target: "self",
    type: "Normal",
  ...
  smackdown: {
    ...
    condition: {
      ...
      onStart(pokemon2) {
        let applies = false;
        if (pokemon2.hasType("Flying") || pokemon2.hasAbility("levitate") || pokemon2.hasAbility("omnitype"))
          applies = true;
  ...
  snatch: {
    ...
    condition: {
      duration: 1,
      onStart(pokemon2) {
        this.add("-singleturn", pokemon2, "Snatch");
      },
  ...
  solarbeam: {
    ...
    onBasePower(basePower, pokemon2, target) {
      ...
      if (pokemon2.effectiveWeather() === "newmoon") {
        this.debug("weakened by weather");
  ...
  sparklingaria: {
    ...
    onAfterMove(source, target, move) {
      for (const pokemon2 of this.getAllActive()) {
        if (pokemon2 !== source && pokemon2.removeVolatile("sparklingaria") && pokemon2.status === "brn" && !source.fainted) {
          pokemon2.cureStatus();
        }
      }
    },
  ...
  spikes: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon2) {
        if (!pokemon2.isGrounded() || pokemon2.hasItem("heavydutyboots"))
          return;
  ...
  spiritaway: {
    ...
    onTry(pokemon2) {
      if (pokemon2.species.name === "Hoopa-Delta-Unleashed") {
        return;
      }
      this.hint("Only a Pokemon whose form is Hoopa-Delta-Unleashed can use this move.");
      if (pokemon2.species.name === "Hoopa-Delta") {
        this.add("-fail", pokemon2, "move: Spirit Away", "[forme]");
        return null;
      }
      this.add("-fail", pokemon2, "move: Spirit Away");
      return null;
    },
    onModifyMove(move, source) {
      if (!source.volatiles["skydrop"]) {
  ...
  spitup: {
    ...
    basePower: 0,
    basePowerCallback(pokemon2) {
      if (!pokemon2.volatiles["stockpile"]?.layers)
        return false;
      return pokemon2.volatiles["stockpile"].layers * 100;
    },
    category: "Special",
    name: "Spit Up",
    pp: 10,
    ...
    onAfterMove(pokemon2) {
      pokemon2.removeVolatile("stockpile");
    },
    secondary: null,
    target: "normal",
    type: "Normal",
  ...
  spotlight: {
    ...
    condition: {
      duration: 1,
      onStart(pokemon2) {
        this.add("-singleturn", pokemon2, "move: Spotlight");
      },
  ...
  stealthcoal: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon2) {
        ...
        const typeMod = this.clampIntRange(pokemon2.runEffectiveness(fireHazard), -6, 6);
        this.damage(pokemon2.maxhp * Math.pow(2, typeMod) / 8);
      }
    },
    secondary: null,
  ...
  steelbeam: {
    ...
    onAfterMove(pokemon2, target, move) {
      if (move.mindBlownRecoil && !move.multihit) {
        const hpBeforeRecoil = pokemon2.hp;
        this.damage(Math.round(pokemon2.maxhp / 2), pokemon2, pokemon2, this.dex.conditions.get("Steel Beam"), true);
        if (pokemon2.hp <= pokemon2.maxhp / 2 && hpBeforeRecoil > pokemon2.maxhp / 2) {
          this.runEvent("EmergencyExit", pokemon2, pokemon2);
        }
      }
  ...
  struggle: {
    ...
    onModifyMove(move, pokemon2, target) {
      move.type = "???";
      this.add("-activate", pokemon2, "move: Struggle");
    },
    struggleRecoil: true,
    secondary: null,
    target: "randomNormal",
  ...
  stuffcheeks: {
    ...
    onDisableMove(pokemon2) {
      if (!pokemon2.getItem().isBerry)
        pokemon2.disableMove("stuffcheeks");
    },
    onTry(source) {
  ...
  swallow: {
    ...
    onHit(pokemon2) {
      ...
      const success = !!this.heal(this.modify(pokemon2.maxhp, healAmount[pokemon2.volatiles["stockpile"].layers - 1]));
      if (!success)
        this.add("-fail", pokemon2, "heal");
      pokemon2.removeVolatile("stockpile");
  ...
  syrupbomb: {
    ...
    condition: {
      ...
      duration: 4,
      onStart(pokemon2) {
        this.add("-start", pokemon2, "Syrup Bomb");
      },
      onResidualOrder: 14,
      onResidual() {
        this.boost({ spe: -1 });
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Syrup Bomb", "[silent]");
      }
    },
    secondary: {
      chance: 100,
    ...
    condition: {
      onStart(pokemon2) {
        if (pokemon2.terastallized)
          return false;
        this.add("-start", pokemon2, "Tar Shot");
      },
  ...
  teatime: {
    ...
    onHitField(target, source, move) {
      ...
      for (const pokemon2 of this.getAllActive()) {
        if (this.runEvent("Invulnerability", pokemon2, source, move) === false) {
          this.add("-miss", source, pokemon2);
        } else if (this.runEvent("TryHit", pokemon2, source, move) && pokemon2.getItem().isBerry) {
          targets.push(pokemon2);
        }
      }
  ...
  technoblast: {
    ...
    onModifyType(move, pokemon2) {
      if (pokemon2.ignoringItem())
        return;
      move.type = this.runEvent("Drive", pokemon2, null, move, "Normal");
    },
    secondary: null,
    target: "normal",
    type: "Normal",
  ...
  telekinesis: {
    ...
    condition: {
      ...
      onUpdate(pokemon2) {
        if (pokemon2.baseSpecies.name === "Gengar-Mega") {
          delete pokemon2.volatiles["telekinesis"];
          this.add("-end", pokemon2, "Telekinesis", "[silent]");
        }
      },
      onResidualOrder: 19,
      onEnd(target) {
  ...
  terablast: {
    ...
    basePower: 80,
    basePowerCallback(pokemon2, target, move) {
      if (pokemon2.terastallized === "Stellar") {
        return 100;
      }
      return move.basePower;
    },
    ...
    onModifyType(move, pokemon2, target) {
      if (pokemon2.terastallized) {
        move.type = pokemon2.teraType;
      }
    },
    onModifyMove(move, pokemon2) {
      if (pokemon2.terastallized && pokemon2.getStat("atk", false, true) > pokemon2.getStat("spa", false, true)) {
        move.category = "Physical";
      }
      if (pokemon2.terastallized === "Stellar") {
  ...
  terastarstorm: {
    ...
    onModifyType(move, pokemon2) {
      if (pokemon2.species.name === "Terapagos-Stellar") {
        move.type = "Stellar";
        if (pokemon2.terastallized && pokemon2.getStat("atk", false, true) > pokemon2.getStat("spa", false, true)) {
          move.category = "Physical";
        }
    ...
    condition: {
      ...
      onBeforeMove(pokemon2, target, move) {
        if (!move.isZ && !move.isMax && move.flags["sound"]) {
          this.add("cant", pokemon2, "move: Throat Chop");
          return false;
        }
      },
      onModifyMove(move, pokemon2, target) {
  ...
  tidyup: {
    ...
    onHit(pokemon2) {
      ...
      return !!this.boost({ atk: 1, spe: 1 }, pokemon2, pokemon2, null, false, true) || success;
    },
    secondary: null,
    target: "self",
    type: "Normal"
  ...
  torment: {
    ...
    condition: {
      ...
      onStart(pokemon2, source, effect) {
        if (pokemon2.volatiles["dynamax"]) {
          delete pokemon2.volatiles["torment"];
          return false;
        }
        if (effect?.id === "gmaxmeltdown")
          this.effectState.duration = 3;
        this.add("-start", pokemon2, "Torment");
      },
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Torment");
      },
      onDisableMove(pokemon2) {
  ...
  transform: {
    ...
    onHit(target, pokemon2) {
      if (!pokemon2.transformInto(target)) {
        return false;
      }
    },
    secondary: null,
  ...
  uproar: {
    ...
    condition: {
      ...
      onLockMove: "uproar",
      onAnySetStatus(status, pokemon2) {
        if (status.id === "slp") {
          if (pokemon2 === this.effectState.target) {
            this.add("-fail", pokemon2, "slp", "[from] Uproar", "[msg]");
          } else {
            this.add("-fail", pokemon2, "slp", "[from] Uproar");
          }
          return null;
        }
      }
      ...
      onModifyMove(move, pokemon2) {
        if (move.secondaries && move.id !== "secretpower") {
          ...
          for (const secondary of move.secondaries) {
            if (pokemon2.hasAbility("serenegrace") && secondary.volatileStatus === "flinch")
              continue;
            if (secondary.chance)
              secondary.chance *= 2;
          }
  ...
  watershuriken: {
    ...
    basePower: 15,
    basePowerCallback(pokemon2, target, move) {
      if (pokemon2.species.name === "Greninja-Ash" && pokemon2.hasAbility("battlebond") && !pokemon2.transformed) {
    ...
    onHit(target, source) {
      if (target.hasType("Grass")) {
        for (const pokemon2 of target.side.pokemon) {
          ...
          if (this.clampIntRange(pokemon2.runEffectiveness(fire), -6, 6) > 0) {
            if (pokemon2.position !== target.position) {
              const immune = ["comatose", "waterveil", "waterbubble"];
              if (!immune.includes(pokemon2.ability) && pokemon2.status === "") {
                pokemon2.status = "brn";
              }
            }
          }
  ...
  wish: {
    ...
    //slotCondition: 'Wish',
    onTry(source) {
      if (!source.side.addSlotCondition(source, "wish"))
        return false;
      if (source.hasAbility("periodicorbit"))
        source.side.addSlotCondition(source, "wishperiodic");
      return this.NOT_FAIL;
    },
    secondary: null,
    target: "self",
  ...
  worryseed: {
    ...
    onHit(pokemon2) {
      const oldAbility = pokemon2.setAbility("insomnia");
      if (oldAbility) {
        this.add("-ability", pokemon2, "Insomnia", "[from] move: Worry Seed");
