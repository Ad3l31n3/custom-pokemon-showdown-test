  ...
  constructor(set, side) {
    this.getDetails = () => {
      ...
      if (this.illusion) {
        ...
        const illusionDetails = displayedSpeciesName + (this.level === 100 ? "" : ", L" + this.level) + (this.illusion.gender === "" ? "" : ", " + this.illusion.gender) + (this.illusion.set.shiny ? ", shiny" : "");
  ...
  */
  ...
  ignoringAbility() {
    ...
    if (this.volatiles["gastroacid"])
      return true;
    if (this.hasItem("Supression Stone"))
      return true;
  ...
  ignoringItem() {
    return !!(this.itemState.knockedOff || this.battle.gen >= 5 && !this.isActive || !this.getItem().ignoreKlutz && this.hasAbility(["klutz", "originalsin"]) || this.volatiles["embargo"] || this.battle.field.pseudoWeather["magicroom"]);
  }
  ...
  transformInto(pokemon, effect) {
    ...
    if (pokemon.fainted || this.illusion || pokemon.illusion || pokemon.volatiles["substitute"] && this.battle.gen >= 5 || pokemon.transformed && this.battle.gen >= 2 || this.transformed && this.battle.gen >= 5 || species.name === "Eternatus-Eternamax" || species.baseSpecies === "Ogerpon" && (this.terastallized || pokemon.terastallized)) {
      return false;
    }
  ...
  setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
    ...
    if (!ignoreImmunities && status.id && !((source?.hasAbility(["corrosion", "cancer"]) || source?.hasItem("plagueorb")) && ["tox", "psn"].includes(status.id)) && !(source?.hasItem("infernoorb") && status.id === "brn") && !(source?.hasItem("stormorb") && status.id === "par")) {
   ...
   */
  setType(newType, enforce = false, source = null, sourceEffect = null) {
    if (!enforce) {
    ...
    const result = this.battle.runEvent("TypeChange", this, source, sourceEffect, [oldTypes, newTypes]);
    if (!result) {
      return false;
    }
    this.types = newTypes;
  ...
  addType(newType, source = null, sourceEffect = null) {
    if (this.terastallized)
      return false;
    const oldTypes = this.types;
  ...
  isGrounded(negateImmunity = false) {
    ...
    if (!negateImmunity && this.hasType("Flying") && !(this.hasType("???") && "roost" in this.volatiles))
      return false;
    if (this.hasAbility(["levitate", "metagaming", "noweaknesses"]) && !this.battle.suppressingAbility(this))
      return null;
