...
class BattleActions {
  ...
  hitStepTypeImmunity(targets, pokemon, move) {
    ...
    const hitResults = [];
    for (const i of targets.keys()) {
      hitResults[i] = move.ignoreImmunity && (move.ignoreImmunity === true || move.ignoreImmunity[move.type]) || targets[i].runImmunity(move.type, !move.smartTarget && !move.canContinue) || move.canContinue || false;
  ...
  getDamage(source, target, move, suppressMessages = false) {
    ...
    if (move.damageCallback)
      return move.damageCallback.call(this.battle, source, target, move);
